...
extract($_REQUEST);
if ($type=="ok"){
	$query_del=mysql_query("delete from areas_noticias where id='$id' limit 1") or die(mysql_error());
	header ("location: ?ok=0");
	exit;
}
$load_query="select * from areas_noticias order by orden ASC";
$bgcolorrep='#E6E6FF';
?>
<html>
<head>
...
<table width="760" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="FFFFFF" class="texto">
  ...
  <tr>
    <td width="443" height="30"><div style="font-family:arial; font-size:12px; font-weight:bold; color: #003366; margin-bottom:15px;"> &nbsp;
            <input name="sUB" type="button"  style="cursor:pointer; font-family:tahoma; font-size:11px; font-weight:normal; color:#fff; height:24px; width:130px; background:#000066" id="sUB" value="NUEVA CATEGORIA" onClick="javascript: iLink('areas_noticias_new.php')">
    </div></td>
      ...
      <?php
		...
		if ($num_rows>=1){
			while ($res=mysql_fetch_array($query)){
				...
				$orden=$res[orden];
				echo '<tr onmouseout="bgColor='."'".$bgcolor."'".'"'.' onmouseover="bgColor='."'".$bgcolorrep."'".'" style=" cursor:pointer" onclick="iLink('."'areas_noticias_new.php?type=2&id=".$id_."'".')">';
				echo '<td height="28" bgcolor="#000066"><div align="center" style=" color:#fff">'.$ini.'</div></td>';
				echo '<td STYLE=" padding-top:5px; padding-bottom:5px; padding-right:5px; padding-left:10px;">'.$nom_area_noti.'</td>';
