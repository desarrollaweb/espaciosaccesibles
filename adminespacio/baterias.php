...
$imgw=130;
if ($type=="ok"){
	mysql_query("delete from baterias WHERE id_bateria='$id' limit 1") or die(mysql_error());
...
if (strlen($txt)>=2){
	$vWhere .=" and CONCAT_WS(' ', a.nom_bateria) like '%".$txt."%'";
}


$load_query="SELECT * FROM baterias ORDER BY id_bateria ASC";
...
<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="FFFFFF" class="texto">
  ...
  <tr>
    ...
    <td width="443" height="42"><div style="font-family:arial; font-size:12px; font-weight:bold; color: #003366;"> &nbsp;
            ...
            <input name="sUB" type="button"  style="cursor:pointer; font-family:tahoma; font-size:10px; font-weight:normal; color:#990000; height:28px; width:130px; background: #FFDDDD" id="sUB" value="NUEVA BATERIA" onClick="javascript: iLink('baterias_new.php')">
  ...
  <tr>
    ...
    <table width="990" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFDDDD" style="font-family:tahoma; font-size:11px; color:#333333">
      ...
      <tr>
        ...
        <td width="67" height="28" bgcolor="#FFE8E8"><div align="center">-</div></td>

        <td width="170" bgcolor="#FFE8E8">&nbsp; <strong>Imagen</strong></td>


        <td width="358" bgcolor="#FFE8E8">&nbsp; <strong>T&iacute;tulo</strong></td>

        <td width="311" bgcolor="#FFE8E8">&nbsp; <strong>Descripci&oacute;n</strong></td>

        <td width="72" bgcolor="#FFE8E8"><div align="center">-</div></td>
      </tr>
      ...
      <?php
		...
		if ($num_rows>=1){
			...
			while ($res=mysql_fetch_array($query)){
				...
				$ini++;
				$id_=$res[id_bateria];
				$nom_bateria=trim($res[nom_bateria]);
				$capacidad=trim($res[capacidad]);
				$reserva=trim($res[reserva]);
				$amperios=trim($res[amperios]);
				$dimensiones=trim($res[dimensiones]);
				$descri='CAPACIDAD: '.$capacidad.'<br> 
				CAPACIDAD DE RESERVA: '.$reserva.'<br>
				CCA a -18.0° C: '.$amperios.'<br>
				DIMENSIONES: '.$dimensiones.'<br>';
				$img_bateria=$path.$res[img_bateria];
				echo '<tr onmouseout="bgColor='."'".$bgcolor."'".'"'.' onmouseover="bgColor='."'".$bgcolorrep."'".'" style=" cursor:pointer" onclick="iLink('."'baterias_new.php?type=2&id=".$id_."'".')">';
				echo '<td height="25" bgcolor="#FFE8E8"><div align="center"">'.$ini.'</div></td>';
				echo '<td><div align="center"><img src="'.$img_bateria.'" width="150"></td>';
				echo '<td><div style=" margin-right:3px; margin-left:3px; margin-bottom:8px; margin-top:8px;">&nbsp; '.$nom_bateria.'</div></td>';
				echo '<td><div style=" margin-right:3px; margin-left:3px; margin-bottom:8px; margin-top:8px;">'.$descri.'</div></td>';
