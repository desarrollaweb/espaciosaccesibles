...
$imgw=130;
if ($type=="ok"){
//	mysql_query("delete from productos where id='$id' limit 1") or die(mysql_error());
...
if (strlen($txt)>=2){
//	$vWhere .=" and CONCAT_WS(' ',a.titulo, a.texto, b.nom_categoria) like '%".$txt."%'";
}

if ($idcategoria>=1){
//	$vWhere .=" and a.idcategoria='".$idcategoria."' ";
}

$load_query="SELECT a.* FROM ipedidos a WHERE !ISNULL(a.id_pedido) ".$vWhere." ORDER BY a.fecha DESC, a.hora DESC";
...
<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="FFFFFF" class="texto">
  ...
  <tr>
    ...
    <td width="760" colspan="2" bgcolor="ffffff"><?php include("top.php") ?></td>

  </tr>

  <tr>

    <td height="50" colspan="2"><table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <table width="990" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFDDDD" style="font-family:tahoma; font-size:11px; color:#333333">
      ...
      <tr>
        ...
        <td width="34" height="28" bgcolor="#FFE8E8"><div align="center">-</div></td>

        <td width="270" bgcolor="#FFE8E8">&nbsp; <strong>Nombres</strong></td>

        <td width="256" bgcolor="#FFE8E8">&nbsp; <strong>Telefonos</strong></td>

        <td width="221" bgcolor="#FFE8E8">&nbsp; <strong>Direccion</strong></td>

        <td width="89" bgcolor="#FFE8E8">&nbsp;<strong>Fecha/Hora</strong></td>
        <td width="106" bgcolor="#FFE8E8"><div align="center">-</div></td>
      </tr>
      ...
      <?php
		...
		if ($num_rows>=1){
			...
			while ($res=mysql_fetch_array($query)){
				...
				$nombre_destinatario=trim($res[nombres]);
				$telefono_destinatario=trim($res[telefonos]);
				$fecha_destinatario=trim($res[fecha_destinatario]);
				$hora_destinatario=trim($res[hora_destinatario]);				
				$distrito_destinatario=$res[distrito];
				$direccion_destinatario=$res[direccion];
				$fecha_destinatario=$res[fecha_destinatario];				
				$hora_destinatario=$res[hora_destinatario];								
				$fecha=$res[fecha];				
				$hora=$res[hora];
				echo '<tr onmouseout="bgColor='."'".$bgcolor."'".'"'.' onmouseover="bgColor='."'".$bgcolorrep."'".'">';
				echo '<td height="25" bgcolor="#FFE8E8"><div align="center"">'.$ini.'</div></td>';
				echo '<td><div style="margin-right:3px; margin-left:6px; margin-bottom:8px; margin-top:8px;">'.$nombres.'</div></td>';
				echo '<td><div style="margin-right:3px; margin-left:6px; margin-bottom:8px; margin-top:8px;">'.$telefono_destinatario.'</div></td>';
				echo '<td><div style="margin-right:3px; margin-left:6px; margin-bottom:8px; margin-top:8px;">'.$direccion_destinatario.'<br>'.$distrito_destinatario.'<br>'.$fecha_destinatario.'<br>'.$hora_destinatario.'</div></td>';
				echo '<td><div align="center">'.$fecha.'<br>'.$hora.'</div></td>';				
				echo '<td><div align="center"><a href="../pdf.php?id='.$id_.'" target="_Blank">VER MAS</a></div></td>';
				echo '</tr>';
				...
				echo '<tr><td height="30" colspan="7">&nbsp;  Lo Sentimos, No hay Registros Encontrados</td></tr>';
